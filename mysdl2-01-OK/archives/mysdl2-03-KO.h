...
Uint8 mysdl_joystick_hat(MySDL* app, int joy_idx, int hat);
...
bool mysdl_init(MySDL* app, const char* title, int width, int height) {
    // Initialize video, joystick, and game controller subsystems
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int a = 0; a < 6; a++) app->joy_axes[i][a] = 0;
        for (int b = 0; b < 32; b++) app->joy_buttons[i][b] = 0;
        for (int h = 0; h < 4; h++) app->joy_hats[i][h] = 0;
    }

    int num_joysticks = SDL_NumJoysticks();
    printf("Detected %d joystick(s) connected.\n", num_joysticks);
    for (int i = 0; i < 4 && i < num_joysticks; i++) {
        app->joysticks[i] = SDL_JoystickOpen(i);
        if (app->joysticks[i]) {
            printf("Opened Joystick %d: %s\n", i, SDL_JoystickName(app->joysticks[i]));
        } else {
...
bool mysdl_poll(MySDL* app) {
    ...
    while (SDL_PollEvent(&e)) {
        ...
        // Debug hook: prints any raw button presses or axis movements to console
        if (e.type == SDL_JOYBUTTONDOWN) {
            printf("[Joystick %d] Button Down: %d\n", e.jbutton.which, e.jbutton.button);
        }
        if (e.type == SDL_JOYAXISMOTION) {
            if (abs(e.jaxis.value) > 10000) { // filter out minor deadzone jitter
                // printf("[Joystick %d] Axis %d Value: %d\n", e.jaxis.which, e.jaxis.axis, e.jaxis.value);
            }
        }
        if (e.type == SDL_JOYHATMOTION) {
            printf("[Joystick %d] Hat %d Position: %d\n", e.jhat.which, e.jhat.hat, e.jhat.value);
        }
    }
    ...
    // Poll live joystick states continuously each frame
    for (int i = 0; i < 4; i++) {
        if (app->joysticks[i]) {
            for (int a = 0; a < 6; a++) {
            ...
            for (int h = 0; h < 4; h++) {
                app->joy_hats[i][h] = SDL_JoystickGetHat(app->joysticks[i], h);
            }
        }
    }
...
Uint8 mysdl_joystick_hat(MySDL* app, int joy_idx, int hat) {
    if (joy_idx < 0 || joy_idx >= 4 || !app->joysticks[joy_idx]) return 0;
    return app->joy_hats[joy_idx][hat];
}
